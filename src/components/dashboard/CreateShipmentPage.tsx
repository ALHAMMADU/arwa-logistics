...
function MethodBgColor(method: string): string {
  if (method === 'AIR') return 'bg-sky-50';
  if (method === 'SEA') return 'bg-violet-50';
  return 'bg-amber-50';
}
...
export default function CreateShipmentPage() {
  ...
  const inputClass = "w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-slate-900";
  const selectClass = "w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-slate-900";
  ...
  if (success) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 mt-6 text-left space-y-3">
          <div className="flex justify-between">
            <span className="text-slate-500">Shipment ID:</span>
            <span className="font-mono font-bold text-slate-900">{success.shipmentId}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Tracking:</span>
            <span className="font-mono font-bold text-emerald-600">{success.trackingNumber}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Destination:</span>
            <span className="text-slate-700">{success.destinationCity}, {success.destinationCountry}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Method:</span>
            <span className="text-slate-700">{SHIPPING_METHOD_LABELS[success.shippingMethod]}</span>
          </div>
          {selectedRate && (
            <div className="flex justify-between">
              <span className="text-slate-500">Total Cost:</span>
              <span className="font-bold text-emerald-600">${selectedRate.totalCost.toFixed(2)}</span>
            </div>
          )}
        </div>
        <div className="flex gap-3 justify-center mt-8">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-white text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors"
          >
            Back to Dashboard
          </button>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <p className="text-slate-500 mb-6">Fill in the shipment details below</p>
      ...
      <div className="flex items-center mb-8">
        {STEPS.map((step, idx) => (
          <React.Fragment key={step.id}>
            ...
            {idx < STEPS.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 rounded ${currentStep > step.id ? 'bg-emerald-400' : 'bg-slate-200'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <select value={form.destinationCountry} onChange={e => { setForm({...form, destinationCountry: e.target.value, routeId: '', destinationCity: ''}); setSelectedRate(null); setRateEstimates([]); }} className={`${selectClass} ${formErrors.destinationCountry ? 'border-red-300' : ''}`} required>
        ...
        {currentStep === 2 && (
          <>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={form.senderPhone} onChange={e => setForm({...form, senderPhone: e.target.value})} className={inputClass} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h3 className="text-lg font-semibold text-slate-900 mb-4">Receiver Information</h3>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  ...
                  <input value={form.receiverPhone} onChange={e => setForm({...form, receiverPhone: e.target.value})} className={inputClass} />
                </div>
        ...
        {currentStep === 3 && (
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            {rateEstimates.length > 0 && (
              <div className="mt-6">
                <h4 className="text-sm font-semibold text-slate-700 mb-3">Available Rates ({rateEstimates.length})</h4>
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-slate-900 text-sm">{rate.routeName}</div>
                            <div className="text-xs text-slate-500">{SHIPPING_METHOD_LABELS[rate.method]} · ${rate.pricePerKg}/kg</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-xl font-bold text-emerald-600">${rate.totalCost.toFixed(2)}</div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                        <div className="bg-slate-50 rounded-lg p-2">
                          ...
                          <div className="font-semibold text-slate-700">${rate.breakdown.baseCost.toFixed(2)}</div>
                        </div>
                        <div className="bg-slate-50 rounded-lg p-2">
                          <div className="text-slate-400">Handling (5%)</div>
                          <div className="font-semibold text-slate-700">${rate.breakdown.handlingFee.toFixed(2)}</div>
                        </div>
                        <div className="bg-slate-50 rounded-lg p-2">
                          <div className="text-slate-400">Insurance (1%)</div>
                          <div className="font-semibold text-slate-700">${rate.breakdown.insurance.toFixed(2)}</div>
                        </div>
                        <div className="bg-slate-50 rounded-lg p-2">
                          <div className="text-slate-400">Customs</div>
                          <div className="font-semibold text-slate-700">${rate.breakdown.customsFee.toFixed(2)}</div>
                        </div>
                      </div>
                    </button>
                  ))}
            ...
            {rateEstimates.length === 0 && comparingRates === false && form.destinationCountry && form.weight && form.shipmentValue && (
              <div className="mt-4 text-center py-6 text-slate-400 text-sm border border-dashed border-slate-200 rounded-xl">
            ...
            {filteredRoutes.length > 0 && (
              <div className="mt-4 pt-4 border-t border-slate-100">
                <label className="block text-sm font-medium text-slate-700 mb-1">Or select a route manually</label>
        ...
        {currentStep === 4 && (
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <h3 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
              <span className="w-7 h-7 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-sm font-bold">4</span>
              Review & Submit
            </h3>
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 mb-6">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-slate-900">{selectedRate.routeName}</div>
                      <div className="text-sm text-slate-500">{SHIPPING_METHOD_LABELS[selectedRate.method]} · {selectedRate.estimatedDays.min}-{selectedRate.estimatedDays.max} days</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-3xl font-bold text-emerald-600">${selectedRate.totalCost.toFixed(2)}</div>
                    <div className="text-xs text-emerald-500">Total Cost</div>
                  </div>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                  <div className="bg-white/70 rounded-lg p-2">
                    <div className="text-slate-500">Base</div>
                    <div className="font-semibold text-slate-800">${selectedRate.breakdown.baseCost.toFixed(2)}</div>
                  </div>
                  <div className="bg-white/70 rounded-lg p-2">
                    <div className="text-slate-500">Handling</div>
                    <div className="font-semibold text-slate-800">${selectedRate.breakdown.handlingFee.toFixed(2)}</div>
                  </div>
                  <div className="bg-white/70 rounded-lg p-2">
                    <div className="text-slate-500">Insurance</div>
                    <div className="font-semibold text-slate-800">${selectedRate.breakdown.insurance.toFixed(2)}</div>
                  </div>
                  <div className="bg-white/70 rounded-lg p-2">
                    <div className="text-slate-500">Customs</div>
                    <div className="font-semibold text-slate-800">${selectedRate.breakdown.customsFee.toFixed(2)}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              <div className="space-y-2">
                <div className="flex justify-between py-1.5 border-b border-slate-100">
                  ...
                  <span className="font-medium text-slate-900">{form.senderName}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-100">
                  <span className="text-slate-500">Receiver</span>
                  <span className="font-medium text-slate-900">{form.receiverName}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-100">
                  <span className="text-slate-500">Destination</span>
                  <span className="font-medium text-slate-900">{form.destinationCity}, {form.destinationCountry}</span>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between py-1.5 border-b border-slate-100">
                  <span className="text-slate-500">Weight</span>
                  <span className="font-medium text-slate-900">{form.weight} kg</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-100">
                  <span className="text-slate-500">Value</span>
                  <span className="font-medium text-slate-900">${form.shipmentValue}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-100">
                  <span className="text-slate-500">Method</span>
                  <span className="font-medium text-slate-900">{SHIPPING_METHOD_LABELS[form.shippingMethod]}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-between items-center">
          <div>
            {currentStep > 1 && (
              <button
                ...
                className="px-5 py-2.5 bg-white text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors"
              >
                Previous
              </button>
            )}
          ...
          <div className="flex gap-3">
            {currentStep < 4 ? (
              ...
              <>
                <button type="button" onClick={() => setCurrentPage('dashboard')} className="px-6 py-2.5 bg-white text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
                  Cancel
                </button>
