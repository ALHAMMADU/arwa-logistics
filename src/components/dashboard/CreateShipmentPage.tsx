...
const STEP_KEYS = ['shipment.stepDestination', 'shipment.stepDetails', 'shipment.stepOptions', 'shipment.stepReview'];
...
export default function CreateShipmentPage() {
  ...
  const handleCompareRates = async () => {
    if (!form.destinationCountry || !form.weight || parseFloat(form.weight) <= 0 || !form.shipmentValue || parseFloat(form.shipmentValue) <= 0) {
      toast.error(t('shipment.fillRequiredCompare'));
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        if (res.data.length === 0) {
          toast.info(t('shipment.noRoutesForCombo'));
        }
      } else {
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.senderName) errors.senderName = t('shipment.senderNameRequired');
    if (!form.receiverName) errors.receiverName = t('shipment.receiverNameRequired');
    if (!form.destinationCountry) errors.destinationCountry = t('shipment.countryRequired');
    if (!form.destinationCity) errors.destinationCity = t('shipment.cityRequired');
    if (!form.weight || parseFloat(form.weight) <= 0) errors.weight = t('shipment.weightRequired');
    if (!form.shipmentValue || parseFloat(form.shipmentValue) <= 0) errors.shipmentValue = t('shipment.valueRequired');
    if (!form.productDescription) errors.productDescription = t('shipment.descriptionRequired');
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{t('shipment.createdSuccess')}</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      >
        <ArrowLeftIcon className="w-4 h-4" /> {t('common.backToDashboard')}
      </button>
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{t('shipment.createNew')}</h1>
      <p className="text-slate-500 dark:text-slate-400 mb-6">{t('shipment.fillDetails')}</p>
        ...
        {STEP_KEYS.map((key, idx) => {
          ...
          return (
            <React.Fragment key={stepId}>
              <button
                type="button"
                onClick={() => canProceedToStep(stepId) && setCurrentStep(stepId)}
              ...
              {idx < STEP_KEYS.length - 1 && (
                <div className={`flex-1 h-0.5 mx-2 rounded ${currentStep > stepId ? 'bg-emerald-400 dark:bg-emerald-600' : 'bg-slate-200 dark:bg-slate-700'}`} />
              )}
            </React.Fragment>
          );
        })}
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {currentStep === 1 && (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
              ...
              {t('shipment.stepDestination')}
            </h3>
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.destination')} *</label>
        ...
        {currentStep === 2 && (
          <>
            {/* Sender Information */}
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t('shipment.sender')}</h3>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.senderName')} *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.senderPhone')}</label>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t('shipment.receiver')}</h3>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.receiverName')} *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.receiverPhone')}</label>
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.receiverAddress')}</label>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.weight')} (kg) *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.dimensions')} - {t('warehouse.lengthCm')}</label>
                  <input type="number" value={form.length} onChange={e => setForm({...form, length: e.target.value})} className={inputClass} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('warehouse.widthCm')}</label>
                  <input type="number" value={form.width} onChange={e => setForm({...form, width: e.target.value})} className={inputClass} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('warehouse.heightCm')}</label>
                  <input type="number" value={form.height} onChange={e => setForm({...form, height: e.target.value})} className={inputClass} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.value')} (USD) *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.product')} *</label>
        ...
        {currentStep === 3 && (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
              ...
              {t('shipment.stepOptions')}
            </h3>
            <div className="grid md:grid-cols-3 gap-4 mb-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.method')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.type')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('nav.warehouses')}</label>
              ...
              >
                {comparingRates ? (
                  <>
                    ...
                    {t('shipment.comparingRates')}
                  </>
                ) : (
                  <>
                    <DollarIcon className="w-5 h-5" />
                    {t('shipment.compareRates')}
                  </>
                )}
              </button>
              {!form.destinationCountry || !form.weight || !form.shipmentValue ? (
                <p className="text-xs text-slate-400 mt-2 text-center">{t('shipment.fillToCompare')}</p>
              ) : null}
            </div>
            ...
            {rateEstimates.length > 0 && (
              <motion.div
                ...
                className="mt-6"
              >
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">{t('shipment.availableRates')} ({rateEstimates.length})</h4>
            ...
            {filteredRoutes.length > 0 && (
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('shipment.orSelectRoute')}</label>
                <select value={form.routeId} onChange={e => { setForm({...form, routeId: e.target.value}); setSelectedRate(null); }} className={selectClass}>
                  <option value="">{t('common.selectRoute')}</option>
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              <div className="space-y-2">
                <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-700">
                  <span className="text-slate-500 dark:text-slate-400">{t('shipment.sender')}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{form.senderName}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-700">
                  <span className="text-slate-500 dark:text-slate-400">{t('shipment.receiver')}</span>
            ...
            {currentStep < 4 ? (
              ...
              <>
                ...
                <button type="submit" disabled={loading} className="px-8 py-2.5 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-500 disabled:opacity-50 transition-colors">
                  {loading ? t('shipment.creatingShipment') : t('shipment.createShipmentButton')}
                </button>
              </>
            )}
          </div>
